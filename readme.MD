...
Change `my-app` to whatever you want your project to be called:

```bash
git clone -b sidebar-layout --single-branch https://github.com/vincepg13/sn-shadkit-template.git my-app
cd my-app
```

### 2️⃣ Remove Template Git History & Reinitialize

This ensures your project starts with a clean `main` branch and no template commit history:
...
### 4️⃣ Configure SDK Authentication

Authenticate with the ServiceNow SDK for the instance you want to develop against:  
...
The Vite dev server now reads the instance URL and authentication details directly from your saved ServiceNow SDK session, so no project `.env` file is required for local development.
